...
public class OrderService extends AccessService implements IOrderService {
	...
	public Order createOrder(Process process, String operator, Map<String, Object> args, 
		...
		//关联具体的业务
		order.setBusType(process.getBusType());
		if (args != null) {
			order.setBusId((String) args.get(SnakerSpecVariableKeys.ORDER_BUS_ID_VAR));
		}
	...
	public void terminate(String orderId, String remark) {
		...
		//强制删除所有的活动任务
		engine.task().delete(tasks.toArray(new Task[tasks.size()]));
		
		doTermination(orderId, remark);
	}
	...
	public void terminate(String orderId, String operator, String remark) {
		AssertHelper.notNull(operator, "operator处理人员id不能为空");
	 ...
	 * @param remark
	 */
	private void doTermination(String orderId, String remark) {
		Order order = access().getOrder(orderId);
		HistoryOrder history = new HistoryOrder(order);
