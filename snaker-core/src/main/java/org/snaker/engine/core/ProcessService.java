...
public class ProcessService extends AccessService implements IProcessService, CacheManagerAware {
	...
	public void updateType(String id, String type) {
		updateType(id, type, null);
	}
	
	public void updateType(String id, String type, String modificator) {
        Process entity = getProcessById(id);
        entity.setType(type);
        if (StringHelper.isNotEmpty(modificator)) {
        	entity.setModificator(modificator);
        	entity.setModifiedTime(DateHelper.getTime());
        }
	...
	public String deploy(InputStream input, String creator) {
		...
		try {
			...
			entity.setContent(new String(bytes,"utf-8"));
	...
	public String deployForFile(String xml) {
		return deployForFile(xml, null);
	}
	
	public String deployForFile(String xml, String creator) {
		try {
	...
	private void cache(Process entity) {
		...
		if(entity.getModel() == null && entity.getContent() != null) {
			...
			entity.setModel(ModelParser.parse(content));
		}
