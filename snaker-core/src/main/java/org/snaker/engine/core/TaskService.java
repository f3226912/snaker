...
public class TaskService extends AccessService implements ITaskService {
	...
	private String[] getTaskActors(TaskModel model, Execution execution) {
		...
		if(StringHelper.isNotEmpty(model.getAssignee())) {
			...
			//如果有全局的AssignmentHandler接口，则执行全局的。 === by gd extend 2016.09.28 start===
			AssignmentHandler globalHandler = ServiceContext.find(AssignmentHandler.class);
			if (globalHandler != null ) {
				if(globalHandler instanceof TaskAssignmentHandler) {
	                assigneeObject = ((TaskAssignmentHandler)globalHandler).assign(model, execution);
	            } else if(globalHandler instanceof Assignment) {
	                assigneeObject = ((Assignment)globalHandler).assign(model, execution);
	            } else {
	                assigneeObject = globalHandler.assign(execution);
	            }
			} 
			//===by gd extend 2016.09.28 end===
		} else if(handler != null) {
			// === by gd extend 2016.09.29 start===
			if(handler instanceof TaskAssignmentHandler) {
                assigneeObject = ((TaskAssignmentHandler)handler).assign(model, execution);
            } else if(handler instanceof Assignment) {
                assigneeObject = ((Assignment)handler).assign(model, execution);
            } else {
                assigneeObject = handler.assign(execution);
            }
			//===by gd extend 2016.09.29 end===
		}
	...
	public Task terminate(String taskId, String operator) {
		return terminate(taskId,operator,null);
	}
	...
	public Task terminate(String taskId, String operator, Map<String, Object> args) {
		...
		task.setVariable(JsonHelper.toJson(args));
		if(!isAllowed(task, operator)) {
			throw new SnakerException("当前参与者[" + operator + "]不允许终止任务[taskId=" + taskId + "]");
		}
		HistoryTask history = new HistoryTask(task);
		history.setFinishTime(DateHelper.getTime());
		//设置为终止状态
		history.setTaskState(STATE_TERMINATION);
		history.setOperator(operator);
		if(history.getActorIds() == null) {
			List<TaskActor> actors = access().getTaskActorsByTaskId(task.getId());
			String[] actorIds = new String[actors.size()];
