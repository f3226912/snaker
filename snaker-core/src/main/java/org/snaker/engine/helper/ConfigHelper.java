...
public class ConfigHelper
{
     ...
     * 常用配置属性文件名称集合.
     */
    private final static String[] PROPERTIES_FILENAMES = {"snaker.properties","properties/snaker.properties"};
        ...
        if(properties == null) {
            synchronized (ConfigHelper.class) {
                if(properties == null) {
                	...
                	//加载文件。
                	for (String fileNames : PROPERTIES_FILENAMES) {
                		appendProperties(fileNames);
					}
                }
            }
        }
    ...
    /**
     * 根据指定的文件名称，从类路径中加载属性文件，追加到Properties对象中。
     * 注意：调用此方法前必须保证Properties对象已经被构造。
     * @param filename 属性文件名称
     */
    private  static void appendProperties(String filename) {
    	InputStream in = null;
