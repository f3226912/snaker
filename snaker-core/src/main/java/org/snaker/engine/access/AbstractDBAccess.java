...
import org.snaker.engine.bo.HistoryOrderBo;
...
public abstract class AbstractDBAccess implements DBAccess {
	...
	protected static final String PROCESS_INSERT = "insert into wf_process (id,name,display_Name,type,instance_Url,state,version,create_Time,creator,bus_Type,org_Id,monitor_Id,process_Desc,modificator,modified_Time) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String PROCESS_UPDATE = "update wf_process set name=?, display_Name=?,state=?,instance_Url=?,create_Time=?,creator=?,bus_Type=?,org_Id=?,monitor_Id=?,process_Desc=?,modificator=?,modified_Time=? where id=? ";
	...
	protected static final String ORDER_INSERT = "insert into wf_order (id,process_Id,creator,create_Time,parent_Id,parent_Node_Name,expire_Time,last_Update_Time,last_Updator,order_No,variable,version,bus_Type,bus_Id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String ORDER_UPDATE = "update wf_order set last_Updator=?, last_Update_Time=?, variable = ?, expire_Time=?, version = version + 1, bus_Type=?, bus_Id=? where id=? and version = ?";
	protected static final String ORDER_DELETE = "delete from wf_order where id = ?";
	protected static final String ORDER_HISTORY_INSERT = "insert into wf_hist_order (id,process_Id,order_State,creator,create_Time,end_Time,parent_Id,expire_Time,order_No,variable,bus_Type,bus_Id,remark) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
	protected static final String ORDER_HISTORY_UPDATE = "update wf_hist_order set order_State = ?, end_Time = ?, variable = ?, remark = ? where id = ? ";
	...
	protected static final String QUERY_PROCESS = "select id,name,display_Name,type,instance_Url,state, content, version,create_Time,creator,bus_Type,org_Id from wf_process ";
	protected static final String QUERY_ORDER = "select o.id,o.process_Id,o.creator,o.create_Time,o.parent_Id,o.parent_Node_Name,o.expire_Time,o.last_Update_Time,o.last_Updator,o.priority,o.order_No,o.variable, o.version, o.bus_Type, o.bus_Id from wf_order o ";
	...
	protected static final String QUERY_HIST_ORDER = "select o.id,o.process_Id,o.order_State,o.priority,o.creator,o.create_Time,o.end_Time,o.parent_Id,o.expire_Time,o.order_No,o.variable, o.bus_Type, o.bus_Id  from wf_hist_order o ";
	...
	protected static final String QUERY_HIST_ORDERBO = "select o.id,o.process_Id,o.order_State,o.priority,o.creator,o.create_Time,o.end_Time,o.parent_Id,o.expire_Time,o.order_No,o.variable,p.display_Name,o.bus_Type, o.bus_Id  from wf_hist_order o ";
	protected static final String QUERY_HIST_ONE_ORDER = "select o.id,o.process_Id,o.order_State,o.priority,o.creator,o.create_Time,o.end_Time,o.parent_Id,o.expire_Time,o.order_No,o.variable,p.display_Name,o.bus_Type, o.bus_Id  from wf_hist_order o ";
	...
	public void saveProcess(Process process) {
		if(isORM()) {
			...
			Object[] args = new Object[]{process.getId(), process.getName(), process.getDisplayName(), process.getType(), 
					...
					process.getBusType(), process.getOrgId(), process.getMonitorId(), process.getProcessDesc(), process.getModificator(),
					process.getModifiedTime()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.INTEGER, 
					...
					Types.CHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR};
	...
	public void updateProcess(Process process) {
		if(isORM()) {
			...
			Object[] args = new Object[]{process.getName(), process.getDisplayName(), process.getState(), 
					...
					process.getBusType(), process.getOrgId(), process.getMonitorId(), process.getProcessDesc(),
					process.getModificator(), process.getModifiedTime()};
			int[] type = new int[]{Types.VARCHAR, Types.VARCHAR, Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, 
					Types.VARCHAR,Types.CHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR,Types.VARCHAR};
	...
	public void saveOrder(Order order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getId(), order.getProcessId(), order.getCreator(), order.getCreateTime(), order.getParentId(), 
                    ...
                    order.getVariable(), order.getVersion(), order.getBusType(), order.getBusId()};
	...
	public void updateOrder(Order order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getLastUpdator(), order.getLastUpdateTime(), order.getVariable(), 
					order.getExpireTime(), order.getId(), order.getVersion(), order.getBusType(), order.getBusId() };
	...
	public void saveHistory(HistoryOrder order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getId(), order.getProcessId(), order.getOrderState(), order.getCreator(), 
					...
					order.getVariable(), order.getBusType(), order.getBusId(), order.getRemark()};
	...
	public void updateHistory(HistoryOrder order) {
		if(isORM()) {
			...
			Object[] args = new Object[]{order.getOrderState(), order.getEndTime(), order.getVariable(), order.getRemark(),order.getId()};
			int[] type = new int[]{Types.INTEGER, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR};
	...
	public HistoryOrderBo getHistOrderBo(String orderId) {
		String where = " left join wf_process p on p.id = o.process_id where o.id = ?";
		return queryObject(HistoryOrderBo.class, QUERY_HIST_ONE_ORDER + where, orderId);
	}
	...
	public List<HistoryOrderBo> getHistoryOrderBos(Page<HistoryOrderBo> page, QueryFilter filter){
		StringBuilder sql = new StringBuilder(QUERY_HIST_ORDERBO);
        sql.append(" left join wf_process p on p.id = o.process_id ");
		sql.append(" where 1=1 ");
		List<Object> paramList = new ArrayList<Object>();
		if(filter.getOperators() != null && filter.getOperators().length > 0) {
        ...
        return queryList2(page, filter, HistoryOrderBo.class, sql.toString(), paramList.toArray());
	}
	...
	public List<WorkItem> getHistoryWorkItems(Page<WorkItem> page, QueryFilter filter) {
	 ...
	 public <T> List<T> queryList2(Page<T> page, QueryFilter filter, Class<T> clazz, String sql, Object... args) {
	        String orderby = StringHelper.buildPageOrder(filter.getOrder(), filter.getOrderBy());
	        String querySQL = sql + orderby;
	        if(page == null) {
	            return queryList2(clazz, querySQL, args);
	        }
	        ...
	        try {
	            ...
	            List<T> list = queryList2(clazz, querySQL, args);
